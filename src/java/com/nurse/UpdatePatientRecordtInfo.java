     ...
     String lab_id = request.getParameter("lab_id");
      ...
      System.out.println(sdf.format(date));
		try {
			...
			int add = statement
					.executeUpdate("insert into patientrecords(PatientID,LabID, BMP, CBC, BP, KCL, PMS, HEARTRATE, PULSE, BMPHASH, CBCHASH, BPHASH, KCLHASH, PMSHASH, HEARTRATEHASH, PULSEHASH, CreatedDate, RequestStatus) values('"
							+ patientDetails + "','" + lab_id  + "','" + BMP + "','" + CBC + "','" + BP + "','" + KCL + "','"
							+ PMS + "','" + heartRate + "','" + pulseRate + "','" + BMPHash + "','" + CBCHash + "','" + BPHash + "','" + KCLHash + "','" + PMSHash + "','" + heartRateHash + "','" + pulseRateHash + "','"+sdf.format(date).toString()+"','" + RequestStatus + "')");
			if (add > 0) {
