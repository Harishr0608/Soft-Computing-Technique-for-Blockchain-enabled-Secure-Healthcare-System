 ...
 * @author THILIPAN
 */
public class ViewPatientHashFiles extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (rs.next()) {
			 RequestDispatcher rd=request.getRequestDispatcher("ViewPatientHashReports.jsp");
                        request.setAttribute("v2",patientId);
                        rd.forward(request, response);
			} else {
				    RequestDispatcher rd=request.getRequestDispatcher("ViewPatientHashFiles.jsp");
